...
- `cocos/2d/renderer/batcher-2d.ts`
  ...
  - 增加 shared binding 快速路径，完整校验只在首次绑定或缓存失效时执行。
...
- `cocos/2d/assembler/sprite/simple.ts`
  - 增加 `createSharedBinding()` / `fillSharedBuffersFast()`。
  - 将 source 的 SIMPLE 局部顶点、UV、材质、纹理状态同步给 copy。
  - copy 使用自己的 `renderData/chunk/color/opacity/worldMatrix`。
  - 增加 source/copy 版本缓存，source 每帧只准备一次，copy 先比较单一总版本，变化时才检查子版本。
...
Batcher2D.walk(copyNode)
    ...
    -> Batcher2D.walkShare(copyNode, sourceNode)
        ...
        -> 首次或失效：assembler.createSharedBinding(sourceRender, copyRender)
        -> 稳定帧：assembler.fillSharedBuffersFast(binding, batcher)
            -> source 变化时才同步 SIMPLE 共享数据
...
```text
fillSharedBuffersFast(binding, batcher)
```
...
## 5. shared binding 与 `fillSharedBuffersFast()` 数据对照
...
当前实现通过 source 的 `batchVersion` 缓存 material、frame、blendHash 和 texture hash。同一个 source 每帧只读取和比较一次这些状态。copy 只在 `batchVersion` 变化时应用 source 的 pass/texture；copy 自己的材质和纹理 dirty 被有意忽略，因为共享语义规定它们必须来自 source。以下变化会触发相应更新：

- copy `nodeDirty/hashDirty`
- source material 变化
- source blendHash 变化
- source frame 变化
...
创建 shared binding 时会主动同步一次 copy color。后续 copy color 变化继续走原版 `Renderable2D.color` setter，直接调用：
...
```text
binding = sourceAssembler.createSharedBinding(sourceSprite, copySprite)
sourceAssembler.fillSharedBuffersFast(binding, this)
```

完整 `walkShare()` 只负责首次绑定和缓存失效后的重建。绑定成功后会缓存 source Sprite、copy Sprite、assembler、RenderData、chunk 和版本状态；稳定帧直接调用 `fillSharedBuffersFast()`。
...
性能上，已经修复了早期 shared copy 每帧强制刷新共享数据的问题，并把 source 的局部顶点、UV、材质和纹理状态检查从“每帧每 copy 一次”降低为“每帧每 source 一次”。稳定 copy 通过 Node 上的 binding 直接进入快速提交，先比较一个 `version` 总版本；只有总版本变化时才检查三个子版本。copy 继续保留自己的 Node、Sprite、renderData/chunk、worldMatrix、color 和 opacity。
...
`version` 是 copy 已应用的 source 总版本。稳定帧只比较这个整数；它不同时，才检查 `rectVersion/uvVersion/batchVersion`，确定真正需要同步的数据。

首次创建 binding 时，总版本和三个子版本都初始化为 `-1`。source 的版本从 `0` 开始，并在首次准备时递增，所以新 copy 一定会完整同步一次。
...
### 12.5 为什么改为 Node binding

早期版本使用两个 WeakMap，以 RenderData 查找 source/copy 状态。虽然能避免把动态字段挂到 RenderData 对象池对象上，但每个 copy 每帧仍要执行两次 WeakMap 查询。

当前实现改为：

```ts
sourceNode._sharedRenderSourceState
copyNode._sharedRenderBinding
```

binding 在首次 `walkShare()` 校验成功后建立，直接保存：

```text
source Sprite
copy Sprite
sourceState
source/copy RenderData
copy chunk
assembler
已应用版本
```

选择 Node 作为 binding 生命周期所有者的原因：

- shared 关系本身已经属于 Node。
- 稳定帧可以直接属性访问，不需要 WeakMap hash lookup。
- remove、换 source 和 destroy 时可以显式置空。
- binding 保存 RenderData 和 chunk 身份，快速检测对象池复用或重新分配。
- 字段在 Node 中声明为 `any`，避免 core 模块反向依赖 2D Sprite 类型。

Node 销毁、`removeCopyChildren()`、copy 改绑 source 时必须同步清理 binding。source 已无 copy 时也会释放 `_sharedRenderSourceState`。
...
然后读取 source 最新结果，与 sourceState 中的上一帧值比较，更新对应子版本。任意子版本变化时，再递增总 `version`。
...
```text
binding 是否仍指向当前 source/copy RenderData 和 chunk？
    否 -> 回退 walkShare，重建 binding

source 总 version 是否不同？
    否 -> 跳过三个子版本检查
    是 -> 再检查：
        rectVersion 不同 -> 复制局部矩形，vertDirty = true
        uvVersion 不同 -> 写入 8 个 UV
        batchVersion 不同 -> updatePass + updateTexture

copy nodeDirty？
    是 -> updateNode；layer setter 会设置 nodeDirty

hashDirty？
    是 -> updateHash
```

copy color 不再每帧轮询四个通道。binding 创建时完整同步一次，后续由原版 `Renderable2D.color` setter 直接调用 `updateColor()`；opacity 继续通过 `uiProps.colorDirty` 级联更新。layer、color、opacity 和 worldMatrix 属于 copy，不能放到 source 版本中。
...
| `walkShare()` 完整校验 | 每个 copy 每帧 | 首次绑定或缓存失效时 |
| source/copy 状态查找 | 每 copy 两次 WeakMap | Node binding 直接访问 |
| copy 共享状态判断 | 多个字段逐项比较 | 1 个总版本；变化时才查子版本 |
| copy color 检查 | 每帧比较 RGBA | setter 驱动，稳定帧不检查 |
...
| copy 改绑 source | 清除旧 binding | 重建 binding，并完整同步新 source |
| source/copy RenderData 或 copy chunk 被重新分配 | binding 身份校验失败 | 回退 `walkShare()`，重建 binding |
...
1. 给 `commitSharedComp()` 增加更窄的 shared fast path，减少与普通组件重复的状态读取。
2. 统计 batch break 原因，确认真正的断批来源后再调整 batch key。
3. 对连续同 source copy 建立 `SharedSpriteGroup`，批量提交 index 和状态。
4. 对长期静止的 group 使用独立静态缓冲，只在成员变化时上传。
5. 如果 Node 遍历成为主热点，在业务层使用轻量实例数据代替一部分 Node/Sprite。
